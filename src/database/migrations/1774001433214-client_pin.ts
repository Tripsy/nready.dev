...
export class ClientPin1774001433214 implements MigrationInterface {
	name = 'ClientPin1774001433214';
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "client" RENAME COLUMN "person_cnp" TO "person_identification_number"`,
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX "IDX_client_cnp_unique" ON "client" ("person_identification_number") WHERE person_identification_number IS NOT NULL AND client_type = 'person'`,
		);
		await queryRunner.query(
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`ALTER TABLE "client" RENAME COLUMN "person_identification_number" TO "person_cnp"`,
		);
		await queryRunner.query(
			`CREATE INDEX "IDX_article_content_unique_per_lang" ON "article_content" ("article_id", "language") `,
		);
		...
		await queryRunner.query(
			`CREATE UNIQUE INDEX "IDX_client_cnp_unique" ON "client" ("person_cnp") WHERE ((person_cnp IS NOT NULL) AND (client_type = 'person'::client_client_type_enum))`,
		);
	}
}
