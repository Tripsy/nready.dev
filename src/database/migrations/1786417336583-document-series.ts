...
export class DocumentSeries1786417336583 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TABLE "document_series" ("id" SERIAL NOT NULL, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP DEFAULT now(), "document_type" "public"."document_series_document_type_enum" NOT NULL, "code" character varying(10) NOT NULL, "year" smallint NOT NULL DEFAULT '0', "start_number" integer NOT NULL DEFAULT '1', "next_number" integer NOT NULL DEFAULT '1', "padding" smallint NOT NULL DEFAULT '0', "format" character varying NOT NULL DEFAULT '{code}-{number}', "notes" text, CONSTRAINT "CHK_7af3c7477454ab008da51fe079" CHECK (("padding" BETWEEN 0 AND 12)), CONSTRAINT "CHK_6e54f807fe46f0a46ca80ad5b2" CHECK (("start_number" > 0 AND "next_number" > 0)), CONSTRAINT "CHK_2f7347644dfd3fbb13ef354228" CHECK (("year" = 0 OR "year" BETWEEN 2000 AND 2100)), CONSTRAINT "PK_187a27e33f96d78f669301cf572" PRIMARY KEY ("id")); COMMENT ON COLUMN "document_series"."code" IS 'Series prefix, e.g. INV'; COMMENT ON COLUMN "document_series"."year" IS 'Calendar year the counter belongs to; 0 = series never resets'; COMMENT ON COLUMN "document_series"."start_number" IS 'First number of the series; a yearly rollover resets to it'; COMMENT ON COLUMN "document_series"."next_number" IS 'Number handed out by the next allocation'; COMMENT ON COLUMN "document_series"."padding" IS 'Zero-padding width applied to {number}; 0 = no padding'; COMMENT ON COLUMN "document_series"."format" IS 'Reference template, placeholders {code} {year} {number}'`,
		);
		await queryRunner.query(
			`CREATE UNIQUE INDEX "IDX_document_series_key" ON "document_series"  ("document_type", "year") `,
		);
		await queryRunner.query(
