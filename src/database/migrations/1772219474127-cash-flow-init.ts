...
export class CashFlowInit1772219474127 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TYPE "public"."cash_flow_category_enum" AS ENUM('customer', 'fuel', 'maintenance', 'tolls', 'employee_salary', 'employee_reimbursement', 'vendor', 'insurance', 'taxes', 'correction', 'refund')`,
		);
		await queryRunner.query(
			`CREATE TYPE "public"."cash_flow_gateway_enum" AS ENUM('direct', 'stripe', 'paypal')`,
		);
		await queryRunner.query(
			`CREATE TYPE "public"."cash_flow_method_enum" AS ENUM('credit_card', 'debit_card', 'paypal', 'cash', 'bank_transfer', 'check', 'crypto', 'gift_card')`,
		);
		await queryRunner.query(
			`CREATE TYPE "public"."cash_flow_status_enum" AS ENUM('pending', 'authorized', 'completed', 'failed', 'refunded', 'partially_refunded', 'canceled', 'expired', 'requires_action')`,
		);
		await queryRunner.query(
  ...
  (
    ...
    OR (parent_id IS NOT NULL AND category_type = 'correction')
		...
		await queryRunner.query(
			`CREATE INDEX "IDX_parent_id" ON "cash_flow" ("parent_id") `,
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`CREATE INDEX "IDX_cash_flow_category_type_created_at" ON "cash_flow" ("category_type", "created_at") `,
		);
		await queryRunner.query(
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`DROP INDEX "public"."IDX_cash_flow_category_type_created_at"`,
		);
		await queryRunner.query(
